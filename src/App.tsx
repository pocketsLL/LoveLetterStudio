...
export default function App() {
  ...
  const artTypesDynamic = useMemo(() => uniq(entries.flatMap((e) => e.artTypes).filter(Boolean)), [entries]);
  const locations = useMemo(() => uniq([...entries.map((e) => e.location).filter(Boolean), ...worldLocations]), [entries, worldLocations]);

  const filtered = useMemo(() => {
    return entries
      .filter((e) => (filterArt === "all" ? true : e.artTypes.includes(filterArt)))
      .filter((e) => (filterLocation === "all" ? true : e.location === filterLocation))
      .filter((e) => (q.trim() ? `${e.name} ${e.location} ${e.artTypes.join(" ")}`.toLowerCase().includes(q.toLowerCase()) : true))
...
<main className="max-w-6xl mx-auto px-4 py-8">
  {route === "#/submit" ? (
    ...
    <Directory
      ...
      filterArt={filterArt} setFilterArt={setFilterArt}
...
function MultiSelectDiscipline({ value, onChange, options, limit=3 }: { value: string[]; onChange: (v: string[]) => void; options: string[]; limit?: number }) {
  ...
  React.useEffect(() => {
    const onClick = (e: MouseEvent) => { if (!containerRef.current) return; if (!containerRef.current.contains(e.target as Node)) setOpen(false); };
  ...
  const label = value.length ? value.join(", ") : "Select disciplines…";
  return (
    <div className="relative" ref={containerRef}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full rounded border border-neutral-300 px-3 py-2 bg-neutral-50 text-left">{label}</button>
      {open && (
...
function LocationAutocomplete({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: string[] }) {
  ...
  function select(val: string) { onChange(val); setQ(val); setOpen(false); }
  return (
    <div className="relative" ref={ref}>
      <input value={q} onChange={(e) => { setQ(e.target.value); onChange(e.target.value); }} onFocus={() => setOpen(true)}
        className="mt-1 w-full rounded border border-neutral-300 px-3 py-2 bg-neutral-50" placeholder="City, Country" />
      {open && (
        <div className="absolute z-20 mt-1 max-h-72 overflow-auto w-full border border-neutral-300 bg-white shadow text-sm">
          {filtered.length === 0 ? (
            ...
            filtered.map((opt) => (
              <button key={opt} type="button" onClick={() => select(opt)} className="block w-full text-left px-3 py-2 hover:bg-neutral-100">{opt}</button>
            ))
          )}
        </div>
      )}
...
function SubmissionForm({ knownLocations }: { knownLocations: string[] }) {
  ...
  function toggleArtType(opt: string) {
    setArtTypes((prev) => {
  ...
  const limitReached = artTypes.length >= 3;
          ...
          <div>
            ...
            <div className="mt-2 grid sm:grid-cols-2 md:grid-cols-3 gap-2">
              {ART_TYPE_OPTIONS.map((opt) => {
                const checked = artTypes.includes(opt);
                return (
                  <label key={opt} className={`flex items-center gap-2 rounded border px-2 py-1 ${checked ? "border-neutral-500" : "border-neutral-300"}`}>
                    <input type="checkbox" checked={checked} onChange={() => toggleArtType(opt)} disabled={!checked && limitReached} />
                    <span className="text-sm">{opt}</span>
                  </label>
                );
              })}
            </div>
            {limitReached && <div className="mt-1 text-xs text-neutral-500">Limit reached (3).</div>}
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm">Where you're from (City, Country)</label>
              <input list="ll-locations" value={location} onChange={(e) => setLocation(e.target.value)} className="mt-1 w-full rounded border border-neutral-300 px-3 py-2 bg-neutral-50" placeholder="City, Country" />
              <datalist id="ll-locations">
                {uniq(knownLocations).map((loc) => <option key={loc} value={loc} />)}
              </datalist>
            </div>
          </div>
